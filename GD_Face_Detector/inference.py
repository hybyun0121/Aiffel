...
from sticker import img2sticker
# hyperparameters
...
def get_landmark(img, rects):
    ...
    '''
    dlib_model_path = os.path.join("dlib_model", "shape_predictor_68_face_landmarks.dat")
...
def main(_):
    ...
    img_sticker = cv2.imread(os.path.join("images/king.png"))
    ...
    if not args_config.camera:
        ...
        prior_index = 0
        while prior_index < len(boxes):
            show_image(img_raw, boxes, classes, scores, img_height_raw, img_width_raw, prior_index,cfg['labels_list'])

            x1, y1, x2, y2 = int(boxes[prior_index][0] * img_width_raw), int(boxes[prior_index][1] * img_height_raw), \
                             int(boxes[prior_index][2] * img_width_raw), int(boxes[prior_index][3] * img_height_raw)
            img_raw = img2sticker(img_raw, img_sticker, x1, y1, x2, y2)
            prior_index += 1
