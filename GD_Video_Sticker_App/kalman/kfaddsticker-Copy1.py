...
def img2sticker_kf(img_orig, img_sticker, detector_hog, landmark_predictor):
    ...
    for dlib_rect, landmark in zip(dlib_rects, list_landmarks):
        ...
        w = dlib_rect.width()
        h = dlib_rect.width()
        print(f"org : x: {x}, y: {y}, w: {w}, z: {h}")
        w_copy = copy.deepcopy(w)
        h_copy = copy.deepcopy(h)
        #x,y,w,h = [ele*2 for ele in [x,y,w,h]]
        print(f"ele : x: {x}, y: {y}, w: {w}, z: {h}")
        break
    # sticker
    img_sticker = cv2.resize(img_sticker, (w_copy, h_copy), interpolation=cv2.INTER_NEAREST)
    ...
    refined_x = x - w // 2
    refined_y = y - h - dlib_rect.width() //2
